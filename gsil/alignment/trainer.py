...
class GSILTrainer(Trainer):
    r"""
    Initialize GSILTrainer.
    ...
    Args:
        ...
        beta (`float`, defaults to 0.1):
            The beta factor in GSIL loss. Higher beta means less divergence from the initial policy.
        loss_type (`str`, defaults to `"sigmoid"`):
            The type of GSIL loss to use. Either `"sigmoid"` the default GSIL loss or `"hinge"` loss from SLiC paper.
    ...
    def __init__(
        ...
        if model_init_kwargs is None:
            ...
            raise ValueError("You passed model_kwargs to the GSILTrainer. But your model is already instantiated.")

        if ref_model_init_kwargs is None:
            ref_model_init_kwargs = {}
        elif not isinstance(ref_model, str):
            raise ValueError(
                "You passed ref_model_kwargs to the GSILTrainer. But your ref_model is already instantiated."
            )

        if isinstance(model, str):
            warnings.warn(
                "You passed a model_id to the GSILTrainer. This will automatically create an "
        ...
        if isinstance(ref_model, str):
            warnings.warn(
                "You passed a ref model_id to the GSILTrainer. This will automatically create an "
                "`AutoModelForCausalLM`"
            )
        ...
        if data_collator is None:
            ...
            if max_length is None:
                warnings.warn(
                    "When using GSILDataCollatorWithPadding, you should set `max_length` in the GSILTrainer's init"
            ...
            if max_prompt_length is None:
                warnings.warn(
                    "When using DataCollatorWithPadding, you should set `max_prompt_length` in the GSILTrainer's init"
            ...
            if max_target_length is None and self.is_encoder_decoder:
                warnings.warn(
                    "When using DataCollatorWithPadding with an encoder decoder architecture, you should set `max_target_length` in the GSILTrainer's init"
                    " it will be set to `128` by default, but you should do it yourself in the future.",
                    UserWarning,
                )
            ...
            if args.remove_unused_columns:
                ...
                # warn users
                warnings.warn(
                    "When using GSILDataCollatorWithPadding, you should set `remove_unused_columns=False` in your TrainingArguments"
    ...
    def gsil_loss(
        ...
        """Compute the GSIL loss for a batch of policy and reference model log probabilities.
        ...
        Args:
            ...
            beta: Temperature parameter for the GSIL loss, typically something in the range of 0.1 to 0.5. We ignore the reference model as beta -> 0.
            reference_free: If True, we ignore the _provided_ reference model and implicitly use a reference model that assigns equal probability to all responses.

        Returns:
            A tuple of three tensors: (losses, real_rewards, generated_rewards).
            The losses tensor contains the GSIL loss for each example in the batch.
        ...
        if self.loss_type == "spin":
            losses = -F.logsigmoid(self.beta * logits)

        elif self.loss_type == "bernoulli_scale_shift":
            loss_real = -F.logsigmoid(real_rewards * self.alpha + self.beta)
            loss_gen = -F.logsigmoid(-(generated_rewards * self.alpha + self.beta))
            losses = loss_real + loss_gen

        elif self.loss_type == "Brier_scale_shift":
            # average logps
            loss_real = F.mse_loss(F.sigmoid(real_rewards*self.alpha+ self.beta), torch.full_like(real_rewards, 1.0))
            loss_gen = F.mse_loss(F.sigmoid(generated_rewards*self.alpha+ self.beta), torch.full_like(generated_rewards, 0))
            losses = loss_real + loss_gen

        elif self.loss_type == "hinge_scale_shift":
            # average logps
            # losses = torch.relu(1 - self.beta * logits)
            loss_real = torch.clamp(1 - (real_rewards*self.alpha + self.beta), min=0)
            loss_gen = torch.clamp(1 + (generated_rewards*self.alpha + self.beta), min=0)
            losses = loss_real + loss_gen

        elif self.loss_type == "KL-IEP":
            loss_real = - (real_rewards * self.alpha + self.beta)
            loss_gen = torch.exp_(generated_rewards * self.alpha + self.beta)
            losses = loss_real + loss_gen

        elif self.loss_type == "LSIF":
            loss_real = -torch.exp(real_rewards * self.alpha + self.beta)
            loss_gen = 1 / 2 * torch.exp(generated_rewards * self.alpha + self.beta) ** 2
            losses = loss_real + loss_gen

        else:
            raise ValueError(f"Unknown loss type: {self.loss_type}. Should be one of ['sigmoid', 'hinge']")

        return losses, real_rewards, generated_rewards, loss_real, loss_gen
    ...
    def get_batch_metrics(
        ...
        """Compute the GSIL loss and other metrics for the given batch of inputs for train or test."""
        metrics = {}
        ...
        ) = self.concatenated_forward(model, batch, average_log_prob)
        with torch.no_grad():
            if self.ref_model is None:
                with self.null_ref_context():
                    (
                        ...
                        opponent_real_logits,
                        opponent_gen_logits,
                    ) = self.concatenated_forward(self.model, batch, average_log_prob)
            else:
                (
                    opponent_real_logps,
                    opponent_generated_logps,
                    opponent_real_logits,
                    opponent_gen_logits,
                ) = self.concatenated_forward(self.ref_model, batch, average_log_prob)

        losses, real_rewards, generated_rewards, loss_real, loss_gen = self.gsil_loss(
            policy_real_logps,
            policy_generated_logps,
            opponent_real_logps,
            opponent_generated_logps,
        ...
        metrics[f"{prefix}rewards/real"] = real_rewards.detach().cpu().mean()
        metrics[f"{prefix}rewards/generated"] = generated_rewards.detach().cpu().mean()
        metrics[f"{prefix}rewards/accuracies"] = reward_accuracies.detach().cpu().mean()
        metrics[f"{prefix}rewards/margins"] = (real_rewards - generated_rewards).detach().cpu().mean()
        ...
        metrics[f"{prefix}logps/oppo_real"] = opponent_real_logps.detach().cpu().mean()
        metrics[f"{prefix}logps/oppo_gen"] = opponent_generated_logps.detach().cpu().mean()
        ...
        metrics[f"{prefix}logits/oppo_generated"] = opponent_real_logits.detach().cpu().mean()
        metrics[f"{prefix}logits/oppo_real"] = opponent_gen_logits.detach().cpu().mean()

        metrics[f"{prefix}loss/real"] = loss_real.detach().cpu().mean()
        metrics[f"{prefix}loss/gen"] = loss_gen.detach().cpu().mean()
